...
router.post('/', (req, res) => {
  try {
    const {
      ...
      flight, returnFlight, hotel, destination,
    ...
    const availFlightsJson       = availableFlights     ? JSON.stringify(availableFlights)     : null;
    const availReturnFlightsJson = availableReturnFlights ? JSON.stringify(availableReturnFlights) : null;
    const availHotelsJson        = availableHotels      ? JSON.stringify(availableHotels)      : null;

    db.prepare(`
      INSERT INTO itineraries (
        user_id, destination_id, destination_name, personality_type,
        itinerary_json, flight_json, return_flight_json, hotel_json,
        destination_json, available_flights_json, available_return_flights_json, available_hotels_json,
        departure_date, return_date, updated_at
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
    ...
    `).run(
      ...
      itineraryJson, flightJson, returnFlightJson, hotelJson,
      destinationJson, availFlightsJson, availReturnFlightsJson, availHotelsJson,
